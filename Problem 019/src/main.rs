fn main() {
    ...
    let solution = sundays_on_first_of_month(&days_in_month);
    println!("{:?}", solution);
}

fn sundays_on_first_of_month(days_in_month: &[u8; 12]) -> u8 {
...
mod tests {
    ...
    fn test_correct_result_problem_18() {
        let days_in_month = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
        assert_eq!(sundays_on_first_of_month(&days_in_month), 171);
    }
}
