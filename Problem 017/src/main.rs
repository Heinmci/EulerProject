...
fn main() {
    let words = get_words_for_nb();
    let solution = get_nb_letters_for_word(1000, words);
    println!("Solution: {}", solution);
}

fn get_nb_letters_for_word(limit: u32, word_mapping: HashMap<u32, &'static str>) -> u32{
...
mod tests {
    ...
    fn test_correct_result() {
        let words = get_words_for_nb();
        assert_eq!(get_nb_letters_for_word(1000, words), 21124);
    }
}